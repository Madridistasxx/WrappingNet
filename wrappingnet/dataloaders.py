...
def normalize(data, ntype="mean-0-std-1", return_minmax=False):
    ...
    if ntype == "range-0,1-mean-0":
        vval_mean = np.repeat(
            np.reshape(np.mean(data, axis=(1)), (-1, 1, 3)), data.shape[1], axis=1
        )
        data = data - vval_mean
        vval_min = np.min(data)
        vval_max = np.max(data)
        ...
        vval_std = np.repeat(
            np.reshape(np.std(data - vval_mean, axis=(1)), (-1, 1, 3)),
            data.shape[1],
            axis=1,
        )
...
class manifold40_dset(InMemoryDataset):
    def __init__(
        self, root, transform=None, train=True, pre_transform=None, pre_filter=None
    ):
    ...
    def process_set(self, dataset):
        ...
        for category in tqdm.tqdm(categories):
            ...
            for path in paths:
                ...
                pos, faces = (
                    torch.tensor(np.asarray(mesh.vertices)).float(),
                    torch.tensor(np.asarray(mesh.triangles)).long(),
                )
