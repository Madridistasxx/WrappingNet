...
sys.path.append(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "../nndistance")
)
...
try:
    ...
    print(
        f"[Warning] CUDA nndistance module not available, using PyTorch fallback. Error: {e}"
    )
    nndistance = None
...
def chamfer_forward_idx(p1, p2):
    if nndistance is not None:
        _, _, _, rec_idx = nndistance(p1.unsqueeze(0), p2.unsqueeze(0))
        return rec_idx

    _, _, _, rec_idx = nndistance_simple(p1.unsqueeze(0), p2.unsqueeze(0))
    return rec_idx
...
def base_loss(pos_base, scale=10, n_sphere=2048, n_base=2048):
    """
    Memory-safe sphere loss.
    Avoids allocating huge pairwise distance matrix.
    """
    device = pos_base.device

    if pos_base.shape[0] > n_base:
        idx = torch.randperm(pos_base.shape[0], device=device)[:n_base]
        pos_base = pos_base[idx]

    pos_sphere = scale * sample_hypersphere(
        d=3,
        n=n_sphere,
    ).to(device)
...
def multiscale_chamfer(pos_list, face_list, p_true, f_true, n=20000):
    d_multi = torch.stack(
        [
            chamfer_face(pos, p_true, face, f_true)
            for pos, face in zip(pos_list, face_list)
        ]
    )
...
def l2(pos_list, face_list, p_true, f_true):
    return torch.mean(
        torch.linalg.norm(pos_list[-1] - p_true[0 : pos_list[-1].shape[0], :], dim=1)
        ** 2
    )
