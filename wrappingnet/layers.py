...
class FaceConv(torch.nn.Module):
    ...
    def forward(self, faces, face_features):
        ...
        conv_feats = face_features[CKP].permute(
            2, 0, 1
        )  # [num_channels, num_faces, num_neighbors]
        ...
        features = [
            ...
            torch.abs(
                conv_feats[..., [num_neighbors - 1] + list(range(num_neighbors - 1))]
                - conv_feats
            ).sum(dim=-1),
...
class Face2Node(torch.nn.Module):
    ...
    def forward(self, pos, faces, face_features):
        ...
        f2 = torch.cat(
            (
                pos[faces[:, 0]] - pos[faces[:, 2]],
                pos[faces[:, 1]] - pos[faces[:, 2]],
                face_features,
            ),
            dim=1,
        )
        ...
        face_features = (
            g0[:, 3:] + g1[:, 3:] + g2[:, 3:]
        ) / 3  # update face features are just averaged among the three

        # Perform aggregation, such that the face features aggregated are relative to the node to be aggregated.
        delta_pos = torch_scatter.scatter_mean(
            torch.cat((g0[:, 0:3], g1[:, 0:3], g2[:, 0:3])),
            faces.T.flatten().unsqueeze(1),
            dim=0,
        )
...
class LoopUnPool(torch.nn.Module):
    ...
    def loop_subdivision(self, pos, faces):
        ...
        hE2E = torch.cat(
            (torch.arange(num_nodes).to(pos.device), hE2E + num_nodes), dim=0
        )
    ...
    def loop_subdivision2(self, pos, faces):
        ...
        unique_edges = (
            current_edges.min(dim=1)[0] * current_edges.max()
            + current_edges.max(dim=1)[0]
        )

        E2F = torch.argsort(unique_edges)
        F2E = torch.zeros_like(E2F)
        F2E[E2F] = torch.div(
            torch.arange(unique_edges.shape[0], device=faces.device),
            2,
            rounding_mode="floor",
        )
        ...
        pos = torch.cat(
            (pos, torch.zeros(nodes_to_add, pos.shape[1], device=pos.device))
        )
    ...
    def forward(self, pos, faces, face_features, mode="nearest"):
        ...
        if mode == "nearest":
            face_features = torch.cat(([face_features] * 4), dim=0)
        elif mode == "bilinear":
            FAF = utils.compute_face_adjacency(faces)
            neighbor_feats = face_features[FAF].permute(
                0, 2, 1
            )  # [num_faces, channels, neighbors]
            face_features = torch.cat(
                ...
                dim=0,
            )  # concatenate along face dimension
        else:
